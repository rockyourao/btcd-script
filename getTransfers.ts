/**
 * 获取 ERC20 Token 的所有 Transfer 事件
 *
 * 使用方法:
 * npx ts-node getTransfers.ts
 *
 * 转换默认的 transfers.json
 * npx ts-node getTransfers.ts --to-csv

 * 或指定文件名
 * npx ts-node getTransfers.ts --to-csv transfers_1212.json
 */
...
// 输出文件
export const OUTPUT_FILE = `data/${network}/transfers.json`;
...
async function main() {
  ...
  try {
    ...
    if (transfers.length > 0) {
      ...
      console.log(`提示: 运行 "npx ts-node src/utils/getTransfers.ts --to-csv" 可将 JSON 转换为 CSV`);
 ...
 * 将 JSON 文件转换为 CSV 文件
 */
function jsonToCsv(jsonFile: string = 'data/transfers.json', csvFile: string = 'data/transfers.csv') {
...
if (args.includes('--to-csv')) {
  // 转换 JSON 到 CSV
  const jsonFile = args[args.indexOf('--to-csv') + 1] || 'data/transfers.json';
