...
// LEND order version guide for frontend/backend routing:
    //
    // | orderType | version | Meaning                                      |
    // |-----------|---------|----------------------------------------------|
    ...
    // | orderType      | version | Meaning                                       |
    // |----------------|---------|-----------------------------------------------|
    // | OBSIDIAN_ORDER | 3       | Feature parity with LEND v3: timeoutRepay     |
    // |                |         | plus pre-minted principal + interest.         |
    // | OBSIDIAN_ORDER | 4       | Add repayment through the order contract      |
    // |                |         | using BTCD balance.                           |
    // | OBSIDIAN_ORDER | 5       | Add increase-loan: top-up principal against   |
    // |                |         | the same collateral.                          |
    //

/**
 * orderVersion: 1: 旧版订单，利息6%，期权1%，30%给nbw，70%给pg，
 * orderVersion: 2: 由keeper解锁btc的订单，利息6%，去掉1%的期权
 * orderVersion: 3: 利息7%，订单借出时就支付给nbw，不支付给pg，但总利息的 6/7 * 70% 需要给pg，需要统计；
 *   每发生一次 OrderDelayed，需再支付与当前利息同口径的一笔，统计时 v3 有效利息 = interestValue * (1 + orderDelayedEvents.length)
 */
      ...
      try {
        ...
        for (const orderId of batchOrderIds) {
          try {
            ...
            console.error(`❌解码订单 ${orderId} 详情失败:`, decodeError.reason, decodeError.code);
          }
        }
        ...
        if (attempt < 2) {
          // console.error(`Multicall 批次 ${i} - ${i + MULTICALL_BATCH_SIZE} 失败，重试 (${attempt + 1}/2)…`, error.reason, error.code, error.error?.body);
          console.error(`Multicall 批次 ${i} - ${i + MULTICALL_BATCH_SIZE} 失败，重试 (${attempt + 1}/2)…`, error.reason, error.code, error.error?.body);
        } else {
          console.error(`Multicall 批次 ${i} - ${i + MULTICALL_BATCH_SIZE} 重试后仍失败:`, error.reason, error.code, error.error?.body);
        }
      }
    }
  }
...
async function fetchOrderClosedAndDelayedEvents(
  ...
  for (let fromBlock = startBlock; fromBlock <= endBlock; fromBlock += BATCH_SIZE) {
    ...
    try {
      ...
      console.error(`获取 OrderClosed/OrderDelayed 事件区块 ${fromBlock}-${toBlock} 失败:`, error.reason, error.code, error.error, error.body);
    }
  }
...
async function getOrderCreatedLogs(
  ...
  for (let fromBlock = startBlock; fromBlock <= endBlock; fromBlock += BATCH_SIZE) {
    const toBlock = Math.min(fromBlock + BATCH_SIZE - 1, endBlock);
    console.log(`区块 ${fromBlock} - ${toBlock}: 开始获取 OrderCreated 事件日志`);
...
function buildOrderStats(
  ...
  const interestOf = (r: OrderRecord) => parseFloat(r.details?.interestValue || '0');
  /** 未显式版本时按旧版 v1（6%+期权）处理；0 亦为链上旧值 */
  const orderVersionOf = (r: OrderRecord) => r.details?.orderVersion ?? 1;
  /**
   * 统计用利息：v3+ 每有一条 OrderDelayed 加计一笔与链上 interestValue 同口径的利息；v1/v2 仍为链上值
   */
  const effectiveInterestOf = (r: OrderRecord): number => {
    const base = interestOf(r);
    if (orderVersionOf(r) < 3) return base;
    const delayCount = r.details?.orderDelayedEvents?.length ?? 0;
    return base * (1 + delayCount);
  };

  const totalInterestValue = btcdRepaidOrdersList.filter(r => orderVersionOf(r) < 3).reduce((sum, r) => sum + effectiveInterestOf(r), 0);
  ...
  /** 仅 v1/legacy 含 1% 期权（利息的 1/6）；v2 无期权；v3+ 为 7% 结构无此项 */
  const totalOptionCost = validOrdersList
    .filter(r => {
      const v = orderVersionOf(r);
      return v < 2;
    })
...
async function main() {
  ...
  if (overdueRankBorrowerTop10.length > 0) {
    ...
    overdueRankBorrowerTop10.forEach((item, i) => {
      console.log(
        `  ${(i + 1).toString().padStart(2, ' ')}. ${item.address} 过期订单数: ${item.count}, ` +
  ...
  console.log(`\n===== 已借出普通订单质押 BTC 排行 Top20 =====`);
  const borrowedCollateralRankTop10 = allRecords.filter(r => r.details?.status === OrderStatus.BORROWED && r.orderType === OrderType.Lending).sort((a, b) => parseFloat(b.details?.realBtcAmount) - parseFloat(a.details?.realBtcAmount)).slice(0, 20);
  ...
  btcdRepaidOrdersTop10.forEach((item, i) => {
    console.log(`  ${(i + 1).toString().padStart(2, ' ')}. ${item.orderId} 还款时间: ${item.details?.borrowerRepaidTimeStr}, BTCD数量: ${formatWithCommas(item.tokenAmount, 2).padStart(9, ' ')}, BTC:${formatWithCommas(item.details?.realBtcAmount, 4).padStart(8, ' ')},  BTC 价格: ${formatWithCommas(item.btcPrice, 2)} 版本: ${item.details?.orderVersion}`);
  });
  ...
  const overdueOrdersTop10 = overdueOrders.slice(0, 20);
  overdueOrdersTop10.forEach((item, i) => {
    console.log(`  ${(i + 1).toString().padStart(2, ' ')}. ${item.orderId} 还款时间: ${timestampToStr(item.details?.deadLinesData?.repayDeadLine)}, BTCD数量: ${formatWithCommas(item.tokenAmount, 2).padStart(9, ' ')}, BTC:${formatWithCommas(item.details?.realBtcAmount, 4).padStart(8, ' ')},  BTC 价格: ${formatWithCommas(item.btcPrice, 2)} 版本: ${item.details?.orderVersion}`);
  });
  ...
  console.log(`  orderVersion < 2 抵押 BTC: ${formatWithCommas(stats.currentBorrowed.orderVersion1.collateral, 4).padStart(8, ' ')} BTC， BTCD 数量: ${formatWithCommas(stats.currentBorrowed.orderVersion1.tokenAmount, 2).padStart(12, ' ')}`);
  console.log(`  orderVersion = 2 抵押 BTC: ${formatWithCommas(stats.currentBorrowed.orderVersion2.collateral, 4).padStart(8, ' ')} BTC， BTCD 数量: ${formatWithCommas(stats.currentBorrowed.orderVersion2.tokenAmount, 2).padStart(12, ' ')}`);
  console.log(`  orderVersion > 2 抵押 BTC: ${formatWithCommas(stats.currentBorrowed.orderVersionGt2.collateral, 4).padStart(8, ' ')} BTC， BTCD 数量: ${formatWithCommas(stats.currentBorrowed.orderVersionGt2.tokenAmount, 2).padStart(12, ' ')}`);
